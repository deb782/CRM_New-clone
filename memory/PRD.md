...
## 2026-06 — BDE journey disposition automation (Piece 1 of 3) [DONE, tested]
- FlowEngine::applyStatus now auto-creates the next BDE call Task on entering CONTACTED (nurture, 24h), NO_RESPONSE (retry, 12h, high), FOLLOWUP_1 (48h), FOLLOWUP_2 (72h), FOLLOWUP_3 (72h) — assigned to lead owner. createFollowUpTask().
- Collateral: entering CONTACTED appends the latest active CpDocument (brochure/price list) link to the customer WhatsApp message.
- Verified via tinker: tasks created + brochure appended.
- PENDING NEXT: (2) BDM Opportunity pipeline — user provided 13 stages ("Opportunity Stage" set): Not Contacted, Initial Call, Follow Up, Site Visit Reschedule, Site Visit Positive, Site Visit No Show, Site Visit - No Immediate Decision, Post Site Visit Followup 1, Post Site Visit Followup 2, Post Site Visit Lost, Pricing Sheet, Negotiations, Final Call. Needs a SEPARATE status group (add group column: lead vs opportunity) that a lead enters after CONVERTED_OPPORTUNITY + BDM transfer (already wired in SiteVisitService). (2b) Engagement loop: auto WhatsApp to lead every 2 days until meeting date OR status change (needs .emergent/crons.yml scheduled task; example Lead A SV+10d=5 msgs, Lead B GMeet+6d=3 msgs). (3) Dashboard UX redesign for Pre-Sales/Sales/Sales-Admin via design_agent — user explicitly wants a distinctive "wow factor", not generic.

## 2026-06-29 — BDM Opportunity Pipeline + Engagement Loop + Sales Cockpit dashboards [DONE, tested 19/19 backend + 100% frontend, iteration_38]
- STATUS GROUPS: lead_statuses.status_group column added (migration 2026_06_29_000000). LeadJourneySeeder rows tagged 'bde' (its prune scoped to group='bde'); new BdmOpportunitySeeder seeds 15 'bdm' statuses across 4 lanes — OPP1 Engagement [OPP_NOT_CONTACTED, OPP_INITIAL_CALL, OPP_FOLLOW_UP], OPP2 Site Visit [OPP_SV_RESCHEDULE, OPP_SV_POSITIVE, OPP_SV_NOSHOW, OPP_SV_NO_DECISION], OPP3 Post Site Visit [OPP_POST_SV_FU1, OPP_POST_SV_FU2, OPP_POST_SV_LOST], OPP4 Closing [OPP_PRICING_SHEET, OPP_NEGOTIATION, OPP_FINAL_CALL, OPP_WON, OPP_LOST]. 13 working stages + Won/Lost terminals. Each maps to pipeline_slug (opportunity/negotiation/won/lost) with allowed_next transition rules. Both seeders now called from DatabaseSeeder.
- HANDOVER (SiteVisitService::schedule): on BDE booking a site visit / google_meet (data.mode) → applyStatus CONVERTED_OPPORTUNITY (fires customer booking-confirmation WA) → handToBdm (least-loaded active sales_bdm) → applyStatus OPP_NOT_CONTACTED (enters BDM pipeline) → creates BDM slot-confirmation Task (high, +4h) + visit-day task → starts engagement loop.
- ENGAGEMENT LOOP: visit_engagements table (migration 2026_06_29_000100) + App\Models\VisitEngagement + App\Services\EngagementService. start() counts send offsets 0,2,4,... strictly before appointment (10d=5, 6d=3), fires the FIRST nudge immediately, schedules the rest every 2 days. dispatchDue() (command crm:engagement-nudge) sends due nudges; stops with stopped_reason status_changed (ANY change from baseline status), appointment_reached, or completed. Idempotent (next_send_at advances 2d per send). Driven by .emergent/crons.yml → POST /crm-api/v1/cron/engagement-nudge (Bearer WEBHOOK_CRON_SECRET in laravel-crm/.env AND backend/.env) hourly; also registered in routes/console.php for Hostinger cron. NOTE: Laravel scheduler does NOT auto-run in preview — the platform cron is the preview driver.
- API: GET /opportunities/board (4 lanes + leads per status), GET /dashboards/{bde|bdm|admin} (SalesDashboardController), GET /journey/statuses?group=bde|bdm|all, POST /cron/engagement-nudge (CronController). All dashboards/board gated permission:leads.view (channel_partner 403).
- DASHBOARDS (design_agent-driven, cockpit.css + sales-cockpit.js, editorial-minimal): sales-cockpit.js wraps CRM.pages.dashboard → role router: sales_bde=Focus Stream (work stack + disposition buttons + context panel), sales_bdm=Opportunity Canvas (stats + upcoming agenda + 13-stage pulse + engagement nudges), sales_head/admin/process_admin=Command Matrix (BDE+BDM funnels, workload balance, SLA strip, gravity heatmap). New nav item 'Opportunity Pipeline' (#/opportunities) = drag-to-move BDM kanban. Cache: app.js v37, sales-cockpit.js v2, cockpit.css v1.
- VERIFIED: tinker (cadence 5/3, status-change stop, handover→sales_bdm, BDM confirm task); curl (all endpoints 200, cron 401/200, board 4 lanes); testing_agent iteration_38 backend 19/19 + frontend 100% (all data-testids, transitions call API + toast); screenshots of all 3 dashboards + board. WhatsApp MOCKED in this preview session.


## 2026-06-29 (wave 2) — BDM stage nudges + Reschedule flow + Admin dashboard revert + Journey full flow [DONE, iteration_39 backend 11/11 + frontend 100%]
- BDM STAGE NUDGES: FlowEngine::createFollowUpTask now maps OPP_PRICING_SHEET / OPP_NEGOTIATION / OPP_FINAL_CALL → high-priority BDM follow-up tasks (map tuples now [title, dueHours, priority]). BdmOpportunitySeeder gives OPP_NEGOTIATION + OPP_FINAL_CALL wa_message + wa_enabled (Pricing already had one). So each closing stage auto-fires a customer WhatsApp + a BDM task.
- RESCHEDULE FLOW: engagement nudges now sent via WhatsAppService::sendInteractive with a 'Reschedule' quick-reply (id 'resched'). WebhookController::handleInboundAutomation: tapping 'resched' → pauses the lead's active VisitEngagement (stopped_reason='reschedule_requested'), creates a high-priority BDM task (meta.reschedule=true), sets conv.bot_state=['reschedule'=>true], replies asking for preferred slot. Next inbound text (guarded ≥3 real chars) → annotateReschedule() logs an activity note + updates the reschedule task title, clears bot_state, thanks the customer. SiteVisitService::reschedule() now calls engagement->start() so the loop auto-restarts on the new date.
- ADMIN DASHBOARD (reverted look, kept name 'Command Matrix'): sales-cockpit.js renderSalesAdmin rewritten to reuse the classic dashboard.css look (dash-hero ring + breakdown, kpi-row, chart-cards) via exposed CRM.dashHelpers (kpi/funnelBars/miniBars/ring/fmt/ymd/hhmm from dashboard.js). Removed SLA strip / Workload / gravity heatmap. Each funnel (BDE + BDM) = top-3 most-populated stages as icon cards (crown/medal/award) + remaining stages as a list with mini bars. Added a full-month calendar. SalesDashboardController::admin() now returns funnel_bde/funnel_bdm/calendar + lost_month (workload/sla removed); calendarVisits() = this month's site visits/meets.
- BDM CALENDAR: renderBDM appends a full-month calendar ('Your month') showing everything (visits + tasks) via SalesDashboardController::bdm().calendar / calendarFull(). Shared monthCalendar() component (prev/next nav, day click → agenda) + global .cal-* styles in cockpit.css.
- JOURNEY BUILDER FULL FLOW: journey.js defaultJourney appends READ-ONLY continuation lanes B1 BDM·Engagement, B2 BDM·Site Visit, B3 BDM·Post Visit, B4 BDM·Closing, PS Post-Sales (with an 'Auto' badge; no add/edit/delete, no Sortable). ensureFullFlow() appends them to any saved journey; compile() + syncFromDom() skip readonly lanes so automation is unchanged; statuses fetched with ?group=all so OPP codes resolve. Cache: dashboard.js v17, sales-cockpit.js v3, cockpit.css v2, journey.js v7.
- Unanswered-at-plan defaults used: funnel top-3 = auto most-populated stages; reschedule = combined (WhatsApp asks for date + BDM task + pause + auto-restart); BDM nudge scope = only Pricing/Negotiation/Final Call. Calendar scope: admin=visits/meets only, BDM/sales=everything.


## 2026-06-29 (wave 3) — Reschedule date parsing + one-tap confirm + calendar drag [DONE, iteration_40 backend 5/5 + frontend 100%]
- DATE PARSING: WebhookController::annotateReschedule now runs the customer's free-text reply through parseWhen() and stores meta.proposed_at (+ preferred_text) on the BDM's reschedule task, retitling it 'Confirm reschedule: <lead> → <date>'. Awaiting-reschedule branch guards too-short/emoji-only replies (re-prompts). Reply message echoes the parsed slot.
- ONE-TAP CONFIRM: new POST /crm-api/v1/reschedules/{task}/confirm (SiteVisitController::confirmReschedule, permission:leads.edit) → reschedules the lead's latest site visit to meta.proposed_at via SiteVisitService::reschedule (restarts the engagement loop) + completes the task. SalesDashboardController::bdm() returns reschedule_requests[{task_id,lead_id,lead_name,proposed_at,preferred_text,visit_id}]; calendar visit items now include visit_id. BDM dashboard shows a 'Reschedule requests' panel with a 'Confirm <date>' button (data-testid confirm-resched-<id>).
- CALENDAR DRAG: monthCalendar() supports HTML5 drag — site-visit agenda items (.cal-item.drag) can be dragged onto any day cell (data-testid cal-day-YYYY-MM-DD); drop keeps the original time on the new date and POSTs /site-visits/{id}/reschedule then refreshes. Wired on both BDM and Admin calendars via opts.onReschedule. Cache: sales-cockpit.js v4, cockpit.css v3.


## 2026-06-29 (CRM.docx backlog — Phase 1: bug cluster) [DONE, self-verified via curl/tinker]
Source doc = ~40-item backlog vs the Hostinger deploy (maroon-goldfinch-862361.hostingersite.com). Many items already exist in current source → Hostinger is running an OLDER build; redeploy + seed resolves several.
- Dashboard stats: added `won_today` to DashboardController adminPayload + salesPayload (Lead status=won, updated_at today); added a "Won Today" KPI card to dashboard.js admin + sales views. All 5 stats (total_leads,new_today,open_tasks,automation_failures,won_today) confirmed returning via GET /dashboard. (Note: 'admin' + sales_head roles now see the Command Matrix wrapper; legacy dashboard is the fallback — both have data.)
- Form 404: FormBuilderController::schema() now returns a graceful 404 JSON ("This form is not available…") instead of the raw ModelNotFound. Added FormsSeeder (default active 'website-lead' form + 4 fields name/phone/email/message) registered in DatabaseSeeder so /public/forms/{slug}/schema works on a fresh deploy. Verified: valid slug 200, bad slug clean 404.
- Auto-scroll bug: leads.js journey panel called scrollIntoView on every 4s poll refresh → page auto-scrolled. Now guarded with wrap.dataset.jtScrolled so it only scrolls on first render.
- Notifications not firing: LeadService now injects NotificationService and creates a 'lead' notification to the assigned owner on capture ("New lead assigned"). Verified a notification row is created. (Groundwork for Phase 8 real-time BDE badge/popup.)
- Website-form source already 'Website Form' in LeadService (stale on Hostinger).
- DEFERRED: "Opportunities pipeline intermittent" — needs a concrete repro/screenshot before a root-cause fix (won't claim a fix without RCA).
...
## 2026-06-30 (Phase 2c — Won Lock · Accounts finance dashboard · Indian currency) [DONE, self-verified]
- **Won Lead Lock**: reaching a Won outcome auto-sets `locked=true` in BOTH transition paths — `FlowEngine::applyStatus` (journey pills / BDM board OPP_WON) and `LeadService::transition` (Kanban pipeline drag). Once locked, journey transitions return ok=false and pipeline transitions abort(423). Frontend leads.js already hides status/chat actions + shows lock banner when `lead.locked`.
- **Accounts finance dashboard**: `accountsPayload()` reshaped to a finance-only view — heading "Accounts & Finance", KPIs [Received MTD, Collections Due, Overdue, GST Billed MTD], panels [Recent Receipts (receipt no/customer/amount/type/date), Upcoming & Overdue Payments]. GST from cost_sheets.gst_amount (MTD).
- **Indian currency (₹1,00,000)**: added `App\Support\Money::group()` / `::inr()` (Indian numbering). Replaced backend `number_format()` in DashboardController, ReceiptService (PDF), CostSheetService, DemandLetterService, AgreementService, PaymentService, BookingService, RunReminders. Frontend already used toLocaleString('en-IN'). Verified: ₹1,23,45,678; receipt PDF shows ₹47,50,000.

## 2026-06-30 (Phase 2d — demo-flow alignment: SV popup, call-log-driven stages, nurture timing) [DONE, self-verified]
- **BDE edit-lock after handover**: `LeadController::assertOwnerOrManager` — front-line execs (tier `exec`, i.e. BDE) can only edit/qualify leads they currently own; after BDE→BDM handover the BDE gets 403. Managers/admin/post-sales unaffected. Verified: BDE 403, BDM 200.
- **Site-visit booked popup (BDM)**: removed `owner_role` from CONVERTED_OPPORTUNITY so `SiteVisitService::handToBdm` owns the transfer + fires a center-screen "Site Visit Booked" popup (data.kind=site_visit) — exactly one popup, no duplicate generic one. notifications.js v3 renders kind-aware popup.
- **Nurture starts on BDM confirm, not on booking**: `SiteVisitService::schedule` no longer starts engagement; `SiteVisitService::confirm` starts it. `EngagementService` STEP_DAYS=3 and stops 3 days before the appointment (STOP_BEFORE_DAYS=3). Verified: 20-day-out SV → 0 sends at booking, 6 sends after confirm.
- **Call-log drives the journey**: `CommunicationController::logCall` now accepts `confirm_visit` (BDM confirms SV → Sales+nurture start), `advance_to` (status code, summary-gated), and marks "Post-Sales journey started" on a Won lead when a postsales user logs a connected call. leads.js v19 call-log UI requires a discussion summary + has a "Confirm site visit" checkbox.
- **Reminders retimed**: SV reminders AppSetting `site_visit_reminder_windows=[4320,360]` (3 days before + day-of); payment reminders `config.integrations.payments.reminder_days=[5]` (5 days before + due + overdue). Seeded in DatabaseSeeder.
- NOTE: nurture + reminders require the platform scheduler/cron running on Hostinger (`* * * * * php artisan schedule:run`).
