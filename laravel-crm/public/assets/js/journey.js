    ...
    return {
      ...
      lanes: [
        { key: 'S1', title: 'Lead Entry', idx: 'STAGE 1', steps: [
          step('send_whatsapp', { template: 'Welcome / Acknowledgement' }),
          step('status_change', { status_code: 'NOT_CONTACTED' }),
          step('task', { title: 'BDE: first contact call', task_type: 'call', due_hours: 2 }),
        ] },
        { key: 'S2', title: 'Qualification', idx: 'STAGE 2', steps: [
          step('task', { title: 'BDE: initial call & verify lead authenticity', task_type: 'call', due_hours: 24 }),
          step('status_change', { status_code: 'CONTACTED' }, ['Junk/Invalid → close', 'No Response → retry']),
          step('condition', { field: 'contact_verified', operator: '=', value: 'true', branches: 'Authentic,Junk,No Response' }),
        ] },
        { key: 'S2F', title: 'FollowUp Stage', idx: 'STAGE 3', steps: [
          step('status_change', { status_code: 'FOLLOWUP_1' }),
          step('send_whatsapp', { template: 'Follow-Up 1' }),
          step('wait', { amount: 2, unit: 'days' }),
          step('status_change', { status_code: 'FOLLOWUP_2' }),
          step('send_whatsapp', { template: 'Follow-Up 2' }),
          step('wait', { amount: 3, unit: 'days' }),
          step('status_change', { status_code: 'FOLLOWUP_3' }, ['No Response → unresponsive']),
        ] },
        { key: 'S3', title: 'Meeting & Site Visit', idx: 'STAGE 4', steps: [
          step('task', { title: 'BDE: book site visit', task_type: 'meeting', due_hours: 48 }),
          step('status_change', { status_code: 'CONVERTED_OPPORTUNITY' }, ['Lost → drop']),
